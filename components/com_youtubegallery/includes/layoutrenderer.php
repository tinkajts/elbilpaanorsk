...
class YoutubeGalleryLayoutRenderer
{
	public static function getValue($fld, $params, &$videolist_row, &$theme_row, $gallery_list, $width, $height, $videoid, $AllowPagination, $total_number_of_rows)//,$title
	{
		...
		switch($fld)
		{
			...
			case 'navigationbar':
				...
				return YoutubeGalleryLayoutRenderer::ClassicNavTable($gallery_list, $navbarwidth, $number_of_columns, $videolist_row, $theme_row, $AllowPagination, $videoid);
			break;
		
			case 'thumbnails':
				//simple list
				return YoutubeGalleryLayoutRenderer::NavigationList($gallery_list, $videolist_row, $theme_row, $AllowPagination, $videoid);
			break;
	...
	public static function render($htmlresult, &$videolist_row, &$theme_row, $gallery_list, $width, $height, $videoid, $total_number_of_rows)
	{
		...
		$fields_generated=array('listname','videotitle','videodescription','videoplayer','navigationbar','thumbnails','count','pagination','width','height','instanceid','videoid','mediafolder');
		$fields_theme=array('bgcolor','cols','cssstyle','navbarstyle','thumbnailstyle','linestyle','listnamestyle','activevideotitlestyle','color1','color2','descr_style','rel','hrefaddon');
	...
	public static function NavigationList($the_gallery_list, &$videolist_row, &$theme_row, $AllowPagination, $videoid)
	{
	...
	public static function ClassicNavTable($the_gallery_list,$width,$number_of_columns, &$videolist_row, &$theme_row, $AllowPagination, $videoid)
	{
		require_once(JPATH_SITE.DS.'components'.DS.'com_youtubegallery'.DS.'includes'.DS.'misc.php');
		$misc=new YouTubeGalleryMisc;
		$misc->videolist_row =$videolist_row;
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						//echo '$listitem[custom_imageurl]='.$listitem['custom_imageurl'].'<br/>';
	...
	public static function renderThumbnailLayout($thumbnail_layout,$listitem,$aHrefLink,$aLink, $videoid,&$theme_row)
	{
		$fields=array('image','link','a','/a','link','title','description',
					  ...
					  'statistics_favoriteCount','viewcount','favcount','keywords','isactive','commentcount','likes','dislikes','channel');
	...
	public static function ShowActiveVideo($gallery_list,$width,$height,$videoid, &$videolist_row, &$theme_row,$videosource='')
	{
		...
		//echo '$VideoRow[custom_imageurl]='.$VideoRow['custom_imageurl'].'<br/>';
