...
class YGPagination extends JObject
{
	...
	protected function _buildDataObject()
	{
		...
		//$WebsiteRoot_old=$juri->getHost();
                //$WebsiteRoot_old2=$juri->root();
                $WebsiteRoot=JURI::base();
		
                //echo '$WebsiteRoot='.$WebsiteRoot.'<br/>';
                //echo '$WebsiteRoot_old='.$WebsiteRoot_old.'<br/>';
                //echo '$WebsiteRoot_old2='.$WebsiteRoot_old2.'<br/>';
		...
		//$URLPath2=$_SERVER['REQUEST_URI']; // example:  /index.php'
                		
		//$pattern = '/[^\pL\pN$-_.+!*\'\(\)\,\{\}\|\\\\\^\~\[\]`\<\>\#\%\"\;\/\?\:\@\&\=\.]/u';
		//$URLPath2 = preg_replace($pattern, '', $URLPath2);
		//$URLPath2 = preg_replace('/"(\n.)+?"/m','', $URLPath2);
		//$URLPath2 = str_replace('"','', $URLPath2);
                
                //$URLPath = JURI::base( true ) ;
		
                $URLPath=$u->getPath();
                
                
                //echo '$URLPath='.$URLPath.'<br/>';
                //echo '$URLPath2='.$URLPath2.'<br/>';
		...
		$URLPath=YoutubeGalleryLayoutRenderer::deleteURLQueryOption($URLPath,$this->prefix.'start');
		...
		if ($this->get('pages.current') > 1)
		{
			...
			$data->previous->link	= $FullPath.$computed_prefix.'start='.$page;
		...
		if ($this->get('pages.current') < $this->get('pages.total'))
		{
			...
			$data->next->base	= $next;
			$data->next->link	= $FullPath.$computed_prefix.'start='.$next;
			if($this->AddAnchor)
				$data->next->link.=$yg_anchor;
			
			$data->end->base	= $end;
			$data->end->link	= $FullPath.$computed_prefix.'start='.$end;
			if($this->AddAnchor)
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
		{
			...
			if ($i != $this->get('pages.current') || $this->_viewall)
			{
				...
				if($offset==0)
					...
					$data->pages[$i]->link	= $FullPath.$computed_prefix.'start='.$offset;
